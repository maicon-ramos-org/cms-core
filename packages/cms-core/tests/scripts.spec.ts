...
describe('a entrada ./scripts', () => {
  ...
  it('o package.json publica as entradas explícitas, incluindo site-reader, e sharp é peer opcional', () => {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'))
    expect(pkg.exports).toEqual({ '.': './src/index.ts', './grafo': './src/grafo/index.ts', './pautas': './src/pautas/index.ts', './scripts': './src/scripts/index.ts', './site-reader': './src/site-reader/index.ts' })
